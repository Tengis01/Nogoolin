...
/** Token when a session exists, else null — never redirects (guest-friendly) */
export async function getOptionalAccessToken(): Promise<string | null> {
  const supabase = createClient();
  const { data } = await supabase.auth.getSession();
...
export async function apiFetch<T>(
  ...
  options: {
    method?: string;
    body?: unknown;
    /** require a session; redirects to /login when absent or on 401 */
    auth?: boolean;
    /**
     * Attach the token only if the user happens to be signed in, and never
     * redirect. For endpoints that accept guests but behave differently when
     * authenticated — POST /inquiries links customer_id (FR-INQ-001).
     */
  ...
  // A guest-capable call must surface its own 401 rather than bouncing the
  // visitor out of a form they were filling in.
  if (res.status === 401 && !options.optionalAuth) redirectToLogin();
